package com.letmecall.rgt.chatbot.ner.tokeninzer;
...
/**
 * A tokenizer that tries to handle sloppy written real life text from short
 * chats.
 * 
...
public class RealLifeChatTokenizer implements Tokenizer {
	...
	private static final Logger logger = LoggerFactory.getLogger(RealLifeChatTokenizer.class);
	...
	public List<String> getTokens(String sentence) {
		logger.debug("getTokens:INVOKED");
		// normalize
		...
		List<String> tokens = new ArrayList<String>(
				Arrays.asList(normalizedSentence.trim().split("" + "\\n+|\\r+|\\t+|\\s+" + "")));
	...
	public String normalizeSentence(String sentence) {
		logger.debug("normalizeSentence:INVOKED");
	...
	private String replaceEndOfLineChars(String sentence) {
		logger.debug("replaceEndOfLineChars:INVOKED");
		String sClean = sentence.replaceAll("(" + "!+|\\?+|;+|\\.(\\s+|$)" + ")", EOS_TOKEN);
	...
	/**
	 * We don't expect them but take them if we have the chance ^^. Replaces
	 * separators like "," with common symbol: SEP.
	 * 
	 * @param sentence - raw input text in lower-case!
	 * @return
	 */
	private String replaceSeparatorChars(String sentence) {
		logger.debug("replaceSeparatorChars:INVOKED");
		return sentence.replaceAll("(" + ",|\\|" + ")", SEPARATOR_TOKEN).trim();
	}
	...
	private String removeSpecialChars(String sentence) {
		logger.debug("removeSpecialChars:INVOKED");
		String sClean = sentence.replaceAll("'(\\w)", "$1").trim(); // what's -> whats
		sClean = sClean.replaceAll("(" + "!|\\?|;|,|:|\\(|\\)|\"|'|\\[|\\]|\\{|\\}" + ")", " ").trim();
		return sClean;
	}

	// ----------------- helpers -------------------

	// Create a regular expression that can filter the common labels (usually not
	// required in result).
	public static String getCommonLabelRegEx(Tokenizer tokenizer, boolean removeDefaultLabel) {
		logger.debug("getCommonLabelRegEx:INVOKED");
